    ...
    .query(async ({ ctx, input }) => {
      ...
      const [moods, sleeps, hydrations, calories, goalData] = await Promise.all([
        ...
        ctx.db.select().from(sleepEntries)
          .where(and(eq(sleepEntries.lookupToken, token), gte(sleepEntries.date, fromStr)))
          .orderBy(sleepEntries.date),
        ctx.db.select().from(hydrationEntries)
          .where(and(eq(hydrationEntries.lookupToken, token), gte(hydrationEntries.date, fromStr)))
          .orderBy(hydrationEntries.date),
        ctx.db.select().from(calorieEntries)
          .where(and(eq(calorieEntries.lookupToken, token), gte(calorieEntries.date, fromStr)))
