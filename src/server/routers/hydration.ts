...
export const hydrationRouter = router({
  today: protectedProcedure.query(async ({ ctx }) => {
    ...
    return entry ?? { glasses: 0, goalGlasses: 8 };
  }),
  ...
  update: protectedProcedure
    .input(z.object({ glasses: z.number().min(0).max(30), goalGlasses: z.number().min(1).max(30).optional() }))
    .mutation(async ({ ctx, input }) => {
      ...
      if (existing) {
        ...
        const [created] = await ctx.db.insert(hydrationEntries)
          .values({ userId: ctx.user.id, date: today, glasses: input.glasses, goalGlasses: input.goalGlasses ?? 8 })
          .returning();
        return created;
      }
    }),
