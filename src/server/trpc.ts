...
export const protectedProcedure = t.procedure.use(({ ctx, next }) => {
  ...
  if (ctx.user.status === "pending") throw new TRPCError({ code: "FORBIDDEN", message: "Aguardando aprovação do administrador." });
  if (ctx.user.status === "banned") throw new TRPCError({ code: "FORBIDDEN", message: "Conta suspensa." });
