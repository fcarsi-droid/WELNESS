...
function IosIcon({ color, emoji, size=42 }: { color: string; emoji: string; size?: number }) {
  return (
    <div style={{ width:size, height:size, borderRadius:"50%", background:color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:size*0.52, flexShrink:0 }}>
      {emoji}
    </div>
  );
}
...
export default function MoodPage() {
  ...
  const upsert = trpc.mood.upsert.useMutation({
    onSuccess: () => { utils.mood.today.invalidate(); utils.mood.history.invalidate(); setEditing(false); setSelected(null); setNote(""); toast.success("Humor registrado!"); },
  });
  const del = trpc.mood.delete.useMutation({
    onSuccess: () => { utils.mood.today.invalidate(); utils.mood.history.invalidate(); toast.success("Registro removido"); },
  });
  ...
  function startEdit() { setSelected(today?.level ?? null); setNote(today?.note ?? ""); setEditing(true); }
    ...
    <div className="fade-in" style={{ maxWidth:700, margin:"0 auto" }}>
      <div style={{ marginBottom:"2rem", display:"flex", alignItems:"center", gap:"0.875rem" }}>
        ...
        <div>
          ...
          <p style={{ margin:0, color:"var(--text-muted)", fontSize:"0.875rem" }}>Como você está hoje?</p>
        </div>
      </div>
      ...
      <div className="card" style={{ padding:"1.75rem", marginBottom:"1.5rem" }}>
        {isLoading ? (
          ...
          <>
            <p style={{ margin:"0 0 1.25rem", fontWeight:600 }}>{editing ? "Atualizar humor de hoje" : "Como você está agora?"}</p>
            ...
            <textarea value={note} onChange={e=>setNote(e.target.value)} placeholder="Alguma observação? (opcional)" maxLength={500}
              style={{ width:"100%", padding:"0.75rem", border:"1.5px solid var(--border)", borderRadius:"0.75rem", fontFamily:"'DM Sans',sans-serif", fontSize:"0.9rem", resize:"vertical", minHeight:80, outline:"none", marginBottom:"1rem", boxSizing:"border-box" }}
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"1.25rem" }}>
            <div style={{ width:72, height:72, borderRadius:"50%", background:currentMood?.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"2.25rem", flexShrink:0 }}>
              {currentMood?.emoji}
            </div>
            <div style={{ flex:1 }}>
              <p style={{ margin:"0 0 0.25rem", fontWeight:600, fontSize:"1.1rem" }}>Hoje você está <span style={{ color:currentMood?.color }}>{currentMood?.label}</span></p>
      ...
      {(history as any[]).length > 0 && (
        <div>
          <h2 style={{ fontSize:"0.75rem", fontFamily:"'DM Sans',sans-serif", fontWeight:700, margin:"0 0 1rem", color:"var(--text-muted)", textTransform:"uppercase", letterSpacing:"0.07em" }}>Últimos 30 dias</h2>
          <div className="card" style={{ padding:"1.25rem", marginBottom:"1rem" }}>
            <div style={{ display:"flex", alignItems:"flex-end", gap:"4px", height:60 }}>
              {(history as any[]).slice(0,30).reverse().map((entry:any, i:number) => {
                const mood = MOODS.find(m=>m.level===entry.level);
                const h = (parseInt(entry.level)/5)*60;
                return <div key={i} title={`${entry.date}: ${mood?.label}`} style={{ flex:1, height:h, background:mood?.color, borderRadius:"3px 3px 0 0", opacity:0.85, minWidth:4 }}/>;
              })}
            </div>
          </div>
          <div style={{ display:"flex", flexDirection:"column", gap:"0.5rem" }}>
            {(history as any[]).slice(0,14).map((entry:any) => {
              const mood = MOODS.find(m=>m.level===entry.level);
              return (
                <div key={entry.id} className="card" style={{ padding:"0.875rem 1.25rem", display:"flex", alignItems:"center", gap:"1rem" }}>
                  <div style={{ width:36, height:36, borderRadius:"50%", background:mood?.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"1.1rem", flexShrink:0 }}>{mood?.emoji}</div>
                  <div style={{ flex:1 }}>
